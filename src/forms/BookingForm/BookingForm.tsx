...
const BookingForm = ({ currentUser, paymentIntent }: Props) => {
  // Razorpay integration will be handled via client-side checkout
...
MM/YY: 12/35 CVC: 123`;
  ...
  const onSubmit = async (formData: BookingFormData) => {
    ...
    if (!ok) {
      showToast({ title: "Payment Error", description: "Failed to load payment SDK", type: "ERROR" });
      return;
    }

    const key = paymentIntent.keyId || (import.meta.env.VITE_RAZORPAY_KEY_ID as string);
    const options: any = {
      key,
      amount: paymentIntent.amount,
      currency: paymentIntent.currency,
      name: "Hotel Booking",
      description: `Booking for ${hotelId}`,
          ...
          <div className="space-y-4">
            ...
            <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
              <p className="text-sm text-gray-700">
                Payments are processed securely via Razorpay. After submitting the form, a secure checkout popup will open to complete the payment.
              </p>
            </div>
